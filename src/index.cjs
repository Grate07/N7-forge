...
function staffMember(member, settings, channel) {
  ...
  return Boolean(
    member.permissions.has(PermissionFlagsBits.ManageChannels) ||
      ...
      (settings.claimRoleId && member.roles.cache.has(settings.claimRoleId)) ||
...
function panelEmbed(settings) {
  const categories = settings.categories.map((category) => `${category.emoji} **${category.name}** — ${category.description}`).join("\n");
  const embed = new EmbedBuilder()
    .setColor(settings.panel.color)
    .setAuthor({ name: "ZYRE MC" })
    .setTitle(settings.panel.title)
    .setDescription(`${settings.panel.description}\n\n**Available ticket types**\n${categories || "No ticket types configured"}\n\n**Before opening a ticket**\nYou can have up to **${settings.maxTicketsPerUser}** open ticket${settings.maxTicketsPerUser === 1 ? "" : "s"} at a time. Please create only one ticket per issue and provide complete information so our staff can assist you faster.`)
...
function canUseCloseAllTicketsCommand(interaction) {
  return Boolean(
    interaction.member?.roles?.cache?.some((role) => CLOSE_ALL_TICKETS_ROLE_IDS.has(role.id))
  );
}
...
function canUseSetCommand(interaction) {
  return Boolean(
    interaction.member?.roles?.cache?.some((role) => SET_COMMAND_ROLE_IDS.has(role.id))
  );
}
...
async function handleAutocomplete(interaction) {
  if (!["set-ticket-category", "set-category", "set-ticket-role"].includes(interaction.commandName)) {
    return;
  }
  const settings = getGuild(interaction.guildId);
...
function startDiscordBot(botLogger) {
  ...
  if (!token) {
    safeLog(botLogger, "warn", "DISCORD_TOKEN is not configured; Discord bot was not started.");
    return;
  }
